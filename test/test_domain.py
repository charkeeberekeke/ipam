...
class TestDomain:
    def __init__(self):
        ...
        self.domain = Domain(domain="Sedgman", groups=self.schema.get_groups("Sedgman"), schema_name="Sedgman")
    ...
    def test_get_node(self):
        n1 = self.domain.add_node(node_type="Region", name="Chile", network="10.64.0.0/12", parent=self.domain.root)
    ...
    def test_child_subnet_validate4(self):
        parent = self.domain.add_node(node_type="Region", name="Unites States", network="10.96.0.0/12", parent=self.domain.root)
    ...
    def test_version(self):
        self.reset()
        eq_(self.domain.version, 1)

    def test_version_2(self):
        self.reset()
        parent = self.domain.add_node(node_type="Region", name="Asia", network="10.16.0.0/12", parent=self.domain.root)
        init_version = self.domain.version
        test_file = "/tmp/tmpfile.xml"
        self.domain.save(xml_file=test_file)
